...
int main() {
  ...
  bool beeActive1 = false, beeActive2 = false, beeActive3 = false,
       beeActive4 = false;
  ...
  while (rm.isOpen()) {
    ...
    while (rm.pollEvent(event)) {
      ...
      if (event.type == Event::KeyPressed &&
        ...
        if (score == 101) {
          ...
          messageText.setOrigin(textRect.left + textRect.width / 2.0,
    ...
    for (int i = 0; i < NUM_BRANCHES; i++) {
      float branchHeight = 1 * 150;
      if (branchPositions[i] == side::LEFT) {
        Branches[i].setPosition(650-220,branchHeight);
        Branches[i].setRotation(180);
      } else if (branchPositions[i] == side::RIGHT) {
        Branches[i].setPosition(950+220, branchHeight);
...
void updateBranches(int seed) {
  // shift positions one place to right
